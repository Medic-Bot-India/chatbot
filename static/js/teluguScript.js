...
function hintoeng(message){
	let apiUrl = `https://api.mymemory.translated.net/get?q=${message}&langpair=te-IN|en-GB&de=prateek.nits.cs@gmail.com`;
		...
		if (val.length < 1) {
			...
			for (i = 0; i <val.length; i++) {
				...
				if (val[i].hasOwnProperty("text")) {
					var BotResponse = ''
					let apiUrl = `https://api.mymemory.translated.net/get?q=${val[i].text}&langpair=en-GB|te-IN&de=prateek.nits.cs@gmail.com`;
					fetch(apiUrl).then(res => res.json()).then(data => {
